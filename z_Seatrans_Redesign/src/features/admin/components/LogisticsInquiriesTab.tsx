...
export function LogisticsInquiriesTab() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const res = await axios.get<PageResponse<LogisticsInquiry>>(
          `${API_BASE}/api/admin/inquiries/total-logistics`,
          {
  ...
  const updateStatus = async (id: number, status: string) => {
    try {
      ...
      const API_BASE = process.env.NEXT_PUBLIC_API_BASE_URL || 'http://localhost:8080'
      await axios.patch(
        `${API_BASE}/api/admin/inquiries/total-logistics/${id}/status`,
        { status },
        { headers: token ? { Authorization: `Bearer ${token}` } : undefined },
      )
  ...
  const deleteInquiry = async (id: number) => {
    ...
    try {
      ...
      const token = authService.getToken()
      const API_BASE = process.env.NEXT_PUBLIC_API_BASE_URL || 'http://localhost:8080'
      await axios.delete(`${API_BASE}/api/admin/inquiries/total-logistics/${id}`, {
        headers: token ? { Authorization: `Bearer ${token}` } : undefined,
      })
