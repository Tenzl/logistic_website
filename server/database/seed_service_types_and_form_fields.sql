...
VALUES
    ...
    (2, 'chartering-ship-broking', 'Chartering & Ship Broking', 'Chartering and broking services', TRUE, NOW(), NOW()),
    (3, 'freight-forwarding', 'Freight Forwarding', 'International freight forwarding', TRUE, NOW(), NOW()),
...
-- Chartering & Ship Broking (2)
INSERT INTO service_form_fields (service_type_id, field_key, label, type, required, placeholder, grid_span, options, position, is_active, created_at, updated_at)
VALUES
    (2, 'charter_type', 'Charter type', 'select', TRUE, 'Select type', 12, '["Time","Voyage","Bareboat"]', 1, TRUE, NOW(), NOW()),
    (2, 'vessel_type', 'Vessel type', 'select', TRUE, 'Select vessel type', 12, '["Bulk Carrier","Container","Tanker","MPP"]', 2, TRUE, NOW(), NOW()),
    (2, 'dwt', 'DWT', 'number', FALSE, 'Deadweight tonnage', 6, NULL, 3, TRUE, NOW(), NOW()),
    (2, 'laycan', 'Laycan', 'date', FALSE, 'Laycan date', 6, NULL, 4, TRUE, NOW(), NOW()),
    (2, 'cargo', 'Cargo', 'text', TRUE, 'Cargo description', 12, NULL, 5, TRUE, NOW(), NOW()),
    (2, 'load_port', 'Load port', 'text', FALSE, 'Port of loading', 6, NULL, 6, TRUE, NOW(), NOW()),
    (2, 'discharge_port', 'Discharge port', 'text', FALSE, 'Port of discharge', 6, NULL, 7, TRUE, NOW(), NOW()),
    (2, 'rate_indication', 'Rate indication', 'text', FALSE, 'Desired rate', 12, NULL, 8, TRUE, NOW(), NOW())
ON DUPLICATE KEY UPDATE label=VALUES(label), type=VALUES(type), required=VALUES(required), placeholder=VALUES(placeholder), grid_span=VALUES(grid_span), options=VALUES(options), position=VALUES(position), is_active=VALUES(is_active), updated_at=NOW();

-- Freight Forwarding (3)
INSERT INTO service_form_fields (service_type_id, field_key, label, type, required, placeholder, grid_span, options, position, is_active, created_at, updated_at)
VALUES
    (3, 'mode', 'Mode', 'select', TRUE, 'Select mode', 6, '["Sea","Air","Road","Rail"]', 1, TRUE, NOW(), NOW()),
    (3, 'incoterms', 'Incoterms', 'select', TRUE, 'Select incoterm', 6, '["FOB","CIF","CFR","EXW","DAP","DDP"]', 2, TRUE, NOW(), NOW()),
    (3, 'origin', 'Origin', 'text', TRUE, 'Departure location', 6, NULL, 3, TRUE, NOW(), NOW()),
    (3, 'destination', 'Destination', 'text', TRUE, 'Arrival location', 6, NULL, 4, TRUE, NOW(), NOW()),
    (3, 'cargo_type', 'Cargo type', 'text', FALSE, 'Type of goods', 6, NULL, 5, TRUE, NOW(), NOW()),
    (3, 'gross_weight_kg', 'Gross weight (kg)', 'number', FALSE, 'Weight in kg', 6, NULL, 6, TRUE, NOW(), NOW()),
    (3, 'volume_cbm', 'Volume (cbm)', 'number', FALSE, 'Volume in cbm', 6, NULL, 7, TRUE, NOW(), NOW()),
    (3, 'ready_date', 'Ready date', 'date', FALSE, 'When cargo is ready', 6, NULL, 8, TRUE, NOW(), NOW()),
    (3, 'dangerous_goods', 'Dangerous goods', 'select', FALSE, 'Select', 6, '["Yes","No"]', 9, TRUE, NOW(), NOW())
ON DUPLICATE KEY UPDATE label=VALUES(label), type=VALUES(type), required=VALUES(required), placeholder=VALUES(placeholder), grid_span=VALUES(grid_span), options=VALUES(options), position=VALUES(position), is_active=VALUES(is_active), updated_at=NOW();
